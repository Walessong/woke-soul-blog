...
function App() {
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      <div className="brand"><span>流动盛宴</span><strong>写作台 v0.1.2</strong></div>
      ...
      <div className="library-summary"><span>文章库</span><b>{visiblePosts.length}</b></div>
...
function Editor({ post, categories, update, save, publish, remove, importImages, textarea, publishing, notice, result, liveLogs }: { post: StudioPost; categories: string[]; update(values: Partial<StudioPost>): void; save(draft?: boolean): Promise<StudioPost | undefined>; publish(): Promise<void>; remove(): Promise<void>; importImages(): Promise<void>; textarea: React.RefObject<HTMLTextAreaElement | null>; publishing: boolean; notice: string; result?: PublishResult; liveLogs: string[] }) {
  return <section className="editor">
    <header className="editor-toolbar"><div className="document-state"><span className={post.draft ? 'status draft' : 'status published'}>{post.draft ? '草稿' : '待发布'}</span><span className="filename">{post.filename || '新文章'}</span></div><div className="toolbar-actions"><button className="text-button danger" disabled={!post.filename} onClick={() => void remove()}>删除</button><button className="secondary" onClick={() => void save()}>保存草稿</button><button className="primary" disabled={publishing} onClick={() => void publish()}>{publishing ? '正在发布…' : '发布文章'}</button></div></header>
